                    ...
                    <form action="{{ url("Home/Service/Add-To-Cart") }}" method="post" enctype="multipart/form-data">
                        <div class="form-header">
                            ...
                            <div class="service-intro">
                                @if(session("user_role") == "Users" && session("user_id") != "")
                                <p class="sm-bold m-b-5">Sprinkler Winterizing Total</p>
                                @endif
                                @if(session("user_role") == "Landscaper" || session("user_role") == "")
                                <p class="sm-bold m-b-5">Sprinkler Winterizing</p>
                                @endif
                                @if(session("user_role") == "Users" && session("user_id") != "")
                                <h3>$<span class="price_value">160.00</span></h3>
                                @endif
                            </div>
                        </div>
...
<?php if (!empty($acres)) { ?>
                                <div class="form-group">
                                    <label for="lawnsize" class="sm-bold title-label">How Big is Your Lawn? <span class="text-danger">*</span></label>
...
<?php } if (!empty($zones)) { ?>
                                <div class="form-group">
                                    <label for="lawnsize" class="sm-bold title-label">How Many Zones do you Have? <span class="text-danger">*</span></label>
                                    <select class="form-control item" id="zonesize">
                                        @foreach($zones as $zone)
                                        <option value="{{ $zone->service_field_price }}">{{ $zone->service_field_value }}</option>
                                        @endforeach
                                    </select>
                                    <input type="hidden" name="no_of_zones" id="no_of_zones">
                                </div>
<?php } ?>
...
<?php } if (!empty($zones)) { ?>
    <script type="text/javascript">
        $(function () {
            var price = parseFloat($("#zonesize").val());
            $("#service_booked_price").val(price);
            var percentage = parseFloat($("#percentage").val());
            price = price + (price * (percentage / 100));
            $(".price_value").html(price.toFixed(2));
            $(".price_value").val(price.toFixed(2));
            $("#no_of_zones").val($("#zonesize option:selected").text());

            $(".item").change(function () {
                price = parseFloat($("#zonesize").val());
                $("#service_booked_price").val(price);
                var percentage = parseFloat($("#percentage").val());
                price = price + (price * (percentage / 100));
                $(".price_value").html(price.toFixed(2));
...
{{-- <script type="text/javascript">
    $(function () {
        var price = parseFloat($("#lawnsize").val()) + parseFloat($("#zonesize").val());
        $("#service_booked_price").val(price);
        var percentage = parseFloat($("#percentage").val());
        price = price + (price * (percentage / 100));
        $(".price_value").html(price.toFixed(2));
        ...
        $(".item").change(function () {
            price = parseFloat($("#lawnsize").val()) + parseFloat($("#zonesize").val());
            $("#service_booked_price").val(price);
            var percentage = parseFloat($("#percentage").val());
            price = price + (price * (percentage / 100));
            $(".price_value").html(price.toFixed(2));
