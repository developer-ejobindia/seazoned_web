        ...
        if ($rating_array->rating_value != 0) {
            ...
            <div class="col-md-4 col-sm-12 col-xs-12">
                ...
                <div class="book-now-form">
                    <form action="{{ url("Home/Service/Add-To-Cart") }}" method="post" enctype="multipart/form-data" id="myForm">
                        <div class="form-header">
                            ...
                            <div class="service-intro">
                                @if(session("user_role") == "Users" && session("user_id") != "")
                                <p class="sm-bold m-b-5">Snow Removal Total</p>
                                @endif
                                @if(session("user_role") == "Landscaper" || session("user_role") == "")
                                <p class="sm-bold m-b-5">Snow Removal</p>
                                @endif
                                @if(session("user_role") == "Users" && session("user_id") != "")
                        ...
                        <div class="form-body">
                            <div class="form-group">
                                <label class="sm-bold title-label">How many cars would fit in your driveway ? <span class="text-danger">*</span></label>
                                <div class="radio-group car-select">
                                    @foreach($car_limit as $key => $value)
                                    <label for="car-select-{{ $value->service_field_value }}"><input type="radio" class="item" name="cars" id="car-select-{{ $value->service_field_value }}" data-field-value="{{ $value->service_field_value }}" value="{{ $value->service_field_price }}" {{ ($key==0) ? "checked" : "" }}><span>{{ $value->service_field_value }}</span></label>
                                    @endforeach
                                </div>
                                <input type="hidden" name="no_of_cars" id="no_of_cars">
                            </div>
                            <div class="form-group">
                                <label class="sm-bold title-label">Driveway type ? <span class="text-danger">*</span></label>
                                <div class="drivewaytype">
                                    <label for="option-2"><input type="radio" id="option-2" name="driveway" class="item" value="{{ $driveway_type[0]->service_field_price }}" data-field-value="{{ $driveway_type[0]->service_field_value }}" checked><img src="{{ asset("default/images/driveway1.png") }}"><span>Straight</span></label>
                                    <label for="option-4"><input type="radio" id="option-4" name="driveway" class="item" value="{{ $driveway_type[1]->service_field_price }}" data-field-value="{{ $driveway_type[1]->service_field_value }}"><img src="{{ asset("default/images/driveway2.png") }}"><span>Circular</span></label>
                                    <label for="option-6"><input type="radio" id="option-6" name="driveway" class="item" value="{{ $driveway_type[2]->service_field_price }}" data-field-value="{{ $driveway_type[2]->service_field_value }}"><img src="{{ asset("default/images/driveway3.png") }}"><span>Incline</span></label>
                                </div>
                                <input type="hidden" name="driveway_type" id="driveway_type">
                            </div>
                            <div class="form-group">
                                <label class="sm-bold title-label">Additional Services? (Select all that apply)<span class="text-danger">*</span></label>
                                <div class="servicesytype">
                                    <label for="services-1"><input type="checkbox" id="services-1" name="servicesytype" class="item" value="{{ $service_type[0]->service_field_price }}" data-field-value="{{ $service_type[0]->service_field_value }}" checked><img src="{{ asset("default/images/servicetype1.png") }}">
                                        <span>Front Door Walkway</span></label>
                                    <label for="services-2"><input type="checkbox" id="services-2" name="servicesytype" class="item" value="{{ $service_type[1]->service_field_price }}" data-field-value="{{ $service_type[1]->service_field_value }}"><img src="{{ asset("default/images/servicetype2.png") }}">
                                        <span>Stairs & Front Landing</span>
                                    </label>
                                    <label for="services-3"><input type="checkbox" id="services-3" name="servicesytype" class="item" value="{{ $service_type[2]->service_field_price }}" data-field-value="{{ $service_type[2]->service_field_value }}"><img src="{{ asset("default/images/servicetype3.png") }}">
                                        <span>Side Door Walkway</span>
                                    </label>
                                </div>
...
<script type="text/javascript">
    $(function () {
        var form = $('#myForm');
        var price = parseFloat(form.find('input[name="cars"]:checked').val()) + parseFloat(form.find('input[name="driveway"]:checked').val()) + parseFloat(form.find('input[name="servicesytype"]:checked').val());
        $("#service_booked_price").val(price);
        var percentage = parseFloat($("#percentage").val());
        price = price + (price * (percentage / 100));
        $("#no_of_cars").val(form.find('input[name="cars"]:checked').attr('data-field-value'));
        $("#driveway_type").val(form.find('input[name="driveway"]:checked').attr('data-field-value'));
        $("#service_type").val(form.find('input[name="servicesytype"]:checked').attr('data-field-value'));
        ...
        $(".item").click(function () {
            ...
            var fav = [];
            var total = 0;
            $('input[name="servicesytype"]:checked').each(function (i) {
                val[i] = $(this).val();
            });
            ...
            var price = parseFloat(form.find('input[name="cars"]:checked').val()) + parseFloat(form.find('input[name="driveway"]:checked').val()) + total;
            $("#service_booked_price").val(price);
            var percentage = parseFloat($("#percentage").val());
            price = price + (price * (percentage / 100));
            $("#no_of_cars").val(form.find('input[name="cars"]:checked').attr('data-field-value'));
            $("#driveway_type").val(form.find('input[name="driveway"]:checked').attr('data-field-value'));
            $('input[name="servicesytype"]:checked').each(function (i) {
                fav[i] = $(this).attr('data-field-value');
            });
            $("#service_type").val(fav);
            $(".price_value").html(price.toFixed(2));
            $(".price_value").val(price.toFixed(2));
        });
    });
