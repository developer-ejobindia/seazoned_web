                    ...
                    <form action="{{ url("Home/Service/Add-To-Cart") }}" method="post" enctype="multipart/form-data">
                        <div class="form-header">
                            ...
                            <div class="service-intro">
                                @if(session("user_role") == "Users" && session("user_id") != "")
                                <p class="sm-bold m-b-5">Pool Cleaning & Upkeep Total</p>
                                @endif
                                @if(session("user_role") == "Landscaper" || session("user_role") == "")
                                <p class="sm-bold m-b-5">Pool Cleaning & Upkeep</p>
                                @endif
                                @if(session("user_role") == "Users" && session("user_id") != "")
                        ...
                        <div class="form-body">
                            <div class="form-group">
                                <label for="lawnsize" class="sm-bold title-label">Pool Water Type? <span class="text-danger">*</span></label>
                                <select class="form-control item" id="watertype">
                                    @foreach($water_type as $type)
                                    <option value="{{ $type->service_field_price }}">{{ $type->service_field_value }}</option>
                                    @endforeach
                                </select>
                                <input type="hidden" name="water_type" id="water_type">
                            </div>
                            <div class="form-group">
                                <label for="lawnsize" class="sm-bold title-label">Include Spa / Hot Tub? <span class="text-danger">*</span></label>
                                <select class="form-control item" id="spa">
                                    @foreach($spa as $value)
                                    <option value="{{ $value->service_field_price }}">{{ $value->service_field_value }}</option>
                                    @endforeach
                                </select>
                                <input type="hidden" name="include_spa" id="include_spa">
                            </div>
                            <div class="form-group">
                                <label for="lawnsize" class="sm-bold title-label">Pool Type? <span class="text-danger">*</span></label>
                                <select class="form-control item" id="pooltype">
                                    @foreach($pool_type as $value)
                                    <option value="{{ $value->service_field_price }}">{{ $value->service_field_value }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group">
                                <label for="lawnsize" class="sm-bold title-label">How Would You Describec the Current State of your Pool ? <span class="text-danger">*</span></label>
                                <select class="form-control item" id="poolstate">
                                    @foreach($pool_state as $value)
                                    <option value="{{ $value->service_field_price }}">{{ $value->service_field_value }}</option>
                                    @endforeach
                                </select>
                                <input type="hidden" name="pool_state" id="pool_state">
                            </div>
...
<script type="text/javascript">
    $(function () {
        var price = parseFloat($("#watertype").val()) + parseFloat($("#spa").val()) + parseFloat($("#pooltype").val()) + parseFloat($("#poolstate").val());
        $("#service_booked_price").val(price);
        var percentage = parseFloat($("#percentage").val());
        price = price + (price * (percentage / 100));
        $("#water_type").val($("#watertype option:selected").text());
        $("#include_spa").val($("#spa option:selected").text());
        $("#pool_type").val($("#pooltype option:selected").text());
        $("#pool_state").val($("#poolstate option:selected").text());
