                    ...
                    <form action="{{ url("Home/Service/Add-To-Cart") }}" method="post" enctype="multipart/form-data">
                        <div class="form-header">
                            ...
                            <div class="service-intro">
                                @if(session("user_role") == "Users" && session("user_id") != "")
                                <p class="sm-bold m-b-5">Lawn Mowing Total</p>
                                @endif 
                                @if(session("user_role") == "Landscaper" || session("user_role") == "")
                                <p class="sm-bold m-b-5">Lawn Mowing</p>
                                @endif    
                                @if(session("user_role") == "Users" && session("user_id") != "")
                        ...
                        <div class="form-body">
                            ...
                            <div class="form-group">
                                <label for="lawnsize" class="sm-bold title-label">How Long is Your Grass?(inches)<span class="text-danger">*</span></label>
                                <select class="form-control item" id="grasssize">
                                    @foreach($grasses as $grass)
                                    <option value="{{ $grass->service_field_price }}">{{ $grass->service_field_value }}</option>
                                    @endforeach
                                </select>
                                <input type="hidden" name="grass_length" id="grass_length">
                            </div>
...
<script type="text/javascript">
    $(function () {
        ...
        var price = parseFloat($("#lawnsize").val()) + parseFloat($("#grasssize").val());
        ...
        $("#grass_length").val($("#grasssize option:selected").text());
        $(".price_value").html(price.toFixed(2));
        $(".price_value").val(price.toFixed(2));

        $(".item").change(function () {
            price = parseFloat($("#lawnsize").val()) + parseFloat($("#grasssize").val());
            $("#service_booked_price").val(price);
            var percentage = parseFloat($("#percentage").val());
            price = price + (price*(percentage/100));
            $("#lawn_area").val($("#lawnsize option:selected").text());
