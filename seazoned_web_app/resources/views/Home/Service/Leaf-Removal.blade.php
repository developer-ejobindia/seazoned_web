                    ...
                    <form action="{{ url("Home/Service/Add-To-Cart") }}" method="post" enctype="multipart/form-data">
                        <div class="form-header">
                            ...
                            <div class="service-intro">
                                @if(session("user_role") == "Users" && session("user_id") != "")
                                <p class="sm-bold m-b-5">Leaf Removal Total</p>
                                @endif   
                                @if(session("user_role") == "Landscaper" || session("user_role") == "")
                                <p class="sm-bold m-b-5">Leaf Removal</p>
                                @endif
                                @if(session("user_role") == "Users" && session("user_id") != "")
                        ...
                        <div class="form-body">
                            ...
                            <div class="form-group">
                                <label for="lawnsize" class="sm-bold title-label">How Long is Your Grass?<span class="text-danger">*</span></label>
                                <select class="form-control item" id="leafsize">
                                    @foreach($leafs as $leaf)
                                    <option value="{{ $leaf->service_field_price }}">{{ $leaf->service_field_value }}</option>
                                    @endforeach
                                </select>
                                <input type="hidden" name="leaf_accumulation" id="leaf_accumulation">
                            </div>
                            ...
                            <p class="text-center m-t-10"><b>Please add your payment info to continue with Booking</b></p>
...
<script type="text/javascript">
    $(function () {
        var price = parseFloat($("#lawnsize").val()) + parseFloat($("#leafsize").val());
        ...
        $("#leaf_accumulation").val($("#leafsize option:selected").text());
        $(".price_value").html(price.toFixed(2));
        $(".price_value").val(price.toFixed(2));

        $(".item").change(function () {
            price = parseFloat($("#lawnsize").val()) + parseFloat($("#leafsize").val());
