...
(function ($, document, window) {
    ...
    $.fn.weatherFeed = function (options) {
        ...
        this.each(function () {
            ...
            var humidity = "";
            var dayDOM = "";
            var codeImgURL = "/seazonedapp/public/default/weather/images/weather/";
                ...
                success: function (response) {
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            ...
                            var weatherListItem = "<h2 class='weather'>" + title + " " + timeZone + " " + "<span class='lastUpdate'>Last Updated at " + time + " " + timeZone + "</span></h2><ul class='weatherSummary'>";
                            var tempSummary;
                            ...
                            for (var i = 0; i < data.query.results.channel.item.forecast.length && i < days; i++) {
                                ...
                                if (i == 0) {
                                    ...
                                    highval=parseInt(((data.query.results.channel.item.forecast[i].high-32)*5)/9);
                                    lowval=parseInt(((data.query.results.channel.item.forecast[i].low-32)*5)/9);
                                    high = "<p class='highTemp'> High: " + highval + " &deg;C</p>";
                                    low = "<p class='lowTemp'> Low: &nbsp;" + lowval + " &deg;C</p>";
                                    ...
                                    highval=parseInt(((data.query.results.channel.item.forecast[i].high-32)*5)/9);
                                    lowval=parseInt(((data.query.results.channel.item.forecast[i].low-32)*5)/9);
                                    high = "<span class='highTemp'> H: " + highval + " &deg;F</span>";
                                    low = "<span class='lowTemp'> L: " + lowval + " &deg;F</span>";
                                    description = "<p>" + data.query.results.channel.item.forecast[i].text + "</p>";
